...
test('任务分组目录重复点击保持在同一栏目', async ({page}) => {
  ...
  await expect.poll(() => page.locator('.main-shell > main').evaluate(element => element.scrollTop)).toBeGreaterThan(0)
  const firstPosition = await page.locator('.main-shell > main').evaluate(element => element.scrollTop)
  await links.nth(1).click()
  await expect.poll(() => page.locator('.main-shell > main').evaluate(element => element.scrollTop)).toBeCloseTo(firstPosition, 0)
})
...
test('移动端放大布局无横向溢出，单栏导航可以收起', async ({page}) => {
  ...
  await expect(page.getByRole('combobox', {name: '界面主题', exact: true})).toBeVisible()
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
...
test('仪表盘设置只含仪表盘项：无启动开关、无删除实例', async ({page}) => {
  ...
  const dialog = page.getByRole('dialog')
  await expect(dialog.getByRole('switch', {name: '卡片等宽'})).toBeVisible()
