...
class TestCopyBackup(ArchiveTestCase):
    ...
    def test_existing_backup_is_not_overwritten(self):
        ...
        self.assertEqual(archive.expire([file], self.bak, 'copy', 'zip', 'alas'), 0)
        with open(file, 'rb') as f:
            self.assertEqual(f.read(), b'new')
        with open(os.path.join(self.bak, 'a.png'), 'rb') as f:
            self.assertEqual(f.read(), b'keep')

    def test_partial_folder_backup_keeps_original_on_retry(self):
        """已有半成品备份不能作为删除完整原目录的依据。"""
        folder = self.make_folder('error', files=('log.txt', '1.png'))
        self.make_folder('bak/error', files=('log.txt',))

        self.assertEqual(archive.expire([folder], self.bak, 'copy', 'zip', 'alas'), 0)
        self.assertTrue(os.path.isfile(os.path.join(folder, '1.png')))
